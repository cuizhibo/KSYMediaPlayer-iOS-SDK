...
#include "ksysdl_vout_overlay_ffmpeg.h"

#include <stdbool.h>
#include <assert.h>
#include "../ksysdl_stdinc.h"
#include "../ksysdl_mutex.h"
#include "../ksysdl_vout_internal.h"
#include "../ksysdl_video.h"
#include "ksysdl_inc_ffmpeg.h"
#include "ksysdl_image_convert.h"
...
SDL_VoutOverlay *SDL_VoutFFmpeg_CreateOverlay(int width, int height, Uint32 format, SDL_Vout *display)
{
    ...
    case SDL_FCC_YV12: {
        ...
        buf_width = KSYALIGN(width, 16); // 1 bytes per pixel for Y-plane
...
#else
        buf_width = KSYALIGN(width, 16); // unknown platform
#endif
        opaque->planes = 3;
        break;
    }
    case SDL_FCC_RV16: {
        ...
        buf_width = KSYALIGN(width, 8); // 2 bytes per pixel
        opaque->planes = 1;
        break;
    }
    case SDL_FCC_RV24: {
    ...
    case SDL_FCC_RV32: {
        ...
        buf_width = KSYALIGN(width, 4); // 4 bytes per pixel
        opaque->planes = 1;
        break;
    }
    default:
    ...
    struct SwsContext **p_sws_ctx, int sws_flags)
{
    ...
    // swscale / direct draw
    /*
    ALOGE("ksy_image_convert w=%d, h=%d, df=%d, dd=%d, dl=%d, sf=%d, sd=%d, sl=%d",
          (int)frame->width,
          (int)frame->height,
          (int)dst_format,
    ...
    if (use_linked_frame) {
        // do nothing
    } else if (ksy_image_convert(frame->width, frame->height,
    ...
    struct SwsContext **p_sws_ctx, int sws_flags)
{
   ...
   // int use_linked_frame = 0;
    ...
    {
            ALOGE("ksy_image_convert_rgb linked_frame");
        if (ksy_image_convert(opaque->linked_frame->width, opaque->linked_frame->height,
            frame->format, frame->data, frame->linesize,
        src_format,  (const uint8_t**) opaque->linked_frame->data, opaque->linked_frame->linesize)) {
        *p_sws_ctx = sws_getCachedContext(*p_sws_ctx,
            opaque->linked_frame->width, opaque->linked_frame->height, opaque->linked_frame->format,
            opaque->linked_frame->width, opaque->linked_frame->height, frame->format, sws_flags, NULL, NULL, NULL);
        if (*p_sws_ctx == NULL) {
            ALOGE("sws_getCachedContext failed");
            return -1;
        }
        sws_scale(*p_sws_ctx,  (const uint8_t**) swscale_dst_pic.data, swscale_dst_pic.linesize,
    ...
    ALOGE("ksy_image_convert_rgb w=%d, h=%d, df=%d, dd=%d, dl=%d",
          (int)overlay->w,
          (int)overlay->h,
          (int)src_format,
          (int)swscale_dst_pic.data[0],
    ...
    if (ksy_image_convert(overlay->w, overlay->h,
        frame->format, frame->data, frame->linesize,
        src_format,  (const uint8_t**) swscale_dst_pic.data, swscale_dst_pic.linesize)) {
        *p_sws_ctx = sws_getCachedContext(*p_sws_ctx,
            overlay->w, overlay->h, src_format, overlay->w, overlay->h,
