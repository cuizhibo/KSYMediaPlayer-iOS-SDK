...
#import "KSYSDLGLShader.h"
#import "KSYSDLGLRender.h"
#import "KSYSDLGLRenderI420.h"
#import "KSYSDLGLRenderRV24.h"
#import "KSYSDLGLRenderNV12.h"
...
static NSString *const g_vertexShaderString = KSY_SHADER_STRING
(
...
@interface KSYSDLGLView()
@property(atomic,strong) NSRecursiveLock *glActiveLock;
@property(atomic) BOOL glActivePaused;
@end

@implementation KSYSDLGLView {
...
- (void)display: (SDL_VoutOverlay *) overlay
{
    ...
    if (![self tryLockGLActive]) {
        NSLog(@"KSYSDLGLView:display: unable to tryLock GL active\n");
        return;
    }
...
{
    if (_context == nil) {
        NSLog(@"KSYSDLGLView: nil EAGLContext\n");
        return;
    }
    ...
    if (![self setupDisplay:overlay]) {
        if ([EAGLContext currentContext] == _context)
            [EAGLContext setCurrentContext:nil];
        NSLog(@"KSYSDLGLView: setupDisplay failed\n");
        return;
    }
...
{
    NSLog(@"KSYSDLGLView:applicationWillEnterForeground: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:NO];
}

- (void)applicationDidBecomeActive
{
    NSLog(@"KSYSDLGLView:applicationDidBecomeActive: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:NO];
}

- (void)applicationWillResignActive
{
    NSLog(@"KSYSDLGLView:applicationWillResignActive: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:YES];
}

- (void)applicationDidEnterBackground
{
    NSLog(@"KSYSDLGLView:applicationDidEnterBackground: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:YES];
}

- (void)applicationWillTerminate
{
    NSLog(@"KSYSDLGLView:applicationWillTerminate: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:YES];
}
