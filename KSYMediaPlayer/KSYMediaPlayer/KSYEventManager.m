...
typedef enum
{
    KSYNotification,                //< Standard NSNotification.
    KSYObjectMethodWithObjectArg,   //< Method with an object argument.
    KSYObjectMethodWithArrayArg     //< Method with an array argument.
} message_type_t;
...
- (BOOL)isEqual:(id)object
{
    ...
    BOOL notificationMatches =
        (otherObject.type == KSYNotification              && [otherObject.name isEqualToString:self.name]) ||
        (otherObject.type == KSYObjectMethodWithArrayArg  && [otherObject.object isEqual:self.object]) ||
        (otherObject.type == KSYObjectMethodWithObjectArg && [otherObject.object isEqual:self.object]);
...
{
    KSYEventManager * self = (__bridge KSYEventManager *)(user_data);
...
@implementation KSYEventManager

+ (id)sharedManager
{
    static dispatch_once_t onceToken;
    static KSYEventManager *defaultManager = nil;
    dispatch_once(&onceToken, ^{
        defaultManager = [[KSYEventManager alloc] init];
    });
...
- (void)startEventLoop {
    for (;;) {
        @autoreleasepool {
            ...
            } else if (message.type == KSYObjectMethodWithArrayArg) {
                NSMutableArray * newArg = nil;

                /* Collapse messages that takes array arg by sending one bigger array */
                for (NSInteger i = [_messageQueue count] - 1; i >= 0; i--) {
                    message_newer = _messageQueue[i];
                    if (message_newer.type == KSYObjectMethodWithArrayArg &&
                        message_newer.target == message.target &&
...
- (void)callOnMainThreadObject:(id)aTarget withMethod:(SEL)aSelector withArgumentAsObject:(id)arg
{
    @autoreleasepool {
        ...
        message.target = aTarget;
        message.object = arg;
        message.type = [arg isKindOfClass:[NSArray class]] ? KSYObjectMethodWithArrayArg : KSYObjectMethodWithObjectArg;
